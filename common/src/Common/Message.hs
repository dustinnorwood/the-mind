...
data WrongCardInfo = WrongCardInfo
  { _wcPlayer :: Text
  , _wcPlayed :: Int
  , _wcMinCard :: Int
  , _wcDiscarded :: [Int]
  } deriving (Eq, Show, Generic)
makeLenses ''WrongCardInfo

emptyPlayerState :: PlayerState
emptyPlayerState = PlayerState False False []

-- Max level based on player count
maxLevel :: Int -> Int
maxLevel n | n <= 2    = 12
           | n == 3    = 10
           | otherwise = 8
...
isGameOver gs = _lives gs <= 0

isGameWon :: GameState -> Bool
isGameWon gs = let numPlayers = M.size (_players gs)
                in _level gs >= maxLevel numPlayers && remainingCards gs == 0
...
tryNextLevel gs =
  if remainingCards gs == 0
    then if isGameWon gs
      then pure gs  -- Game is won, don't advance
      else nextLevel gs
    else pure gs
...
-- | Play a card and return the new game state + optional wrong card info
playCard :: Text -> GameState -> (GameState, Maybe WrongCardInfo)
playCard name gs =
  let playerCard = getTopCardForPlayer name gs
   ...
   in if nextCard == playerCard
      then (gs', Nothing)
      else let gsWrong = gs & lives -~ 1
                            & trimCards playerCard
               wcInfo = WrongCardInfo
                 { _wcPlayer = name
                 , _wcPlayed = fromMaybe 0 playerCard
                 , _wcMinCard = fromMaybe 0 nextCard
                 , _wcDiscarded = []
                 }
            in (gsWrong, Just wcInfo)
...
myGameState name gs@GameState{..} =
  let _myName = name
      ...
      _myGameOver = isGameOver gs
      _myGameWon = isGameWon gs
   in MyGameState{..}
...
instance ToJSON WrongCardInfo where toEncoding = genericToEncoding options
instance FromJSON WrongCardInfo where parseJSON = genericParseJSON options
